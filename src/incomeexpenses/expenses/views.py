...
def index(request):
    ...
    currency = UserPreferences.objects.filter(user=request.user)
    context = {
        'expenses': expenses,
...
def add_expenses(request):
    ...
    if request.method == 'POST':
        ...
        if not date:
            messages.error(request, 'Enter a valid date')
            return render(request, 'expenses/add_expenses.html', context)
        expenses = Expense.objects.create(amount=amount,
...
#     response = HttpResponse(content_type='application/ms-excel')
#     response['Content-Disposition'] = 'inlinr; attachment; filename=Expenses' + \
